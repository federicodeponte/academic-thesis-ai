...
def main():
    ...
    if title_success > 0:
        ...
        if title_fail > 0:
            print(f"   ⚠️  {title_fail} titles could not be scraped (timeouts, errors)")
    ...
    if metadata_success > 0:
        ...
        if metadata_fail > 0:
            print(f"   ⚠️  {metadata_fail} metadata items could not be scraped")
    ...
    filter_report = filter_obj.generate_report(filter_stats, "Opensource Thesis")
    print(filter_report)
...
{paper_for_enhancement}""",
    ...
    try:
        ...
        fallback_path = Path("/tmp") / "FINAL_THESIS_OPENSOURCE_BACKUP.md"
        try:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if validation_result.passed:
                print(f"✅ PDF validation passed ({validation_result.page_count} pages, {validation_result.engine_used})")
            else:
                print(f"❌ PDF validation failed:")
                for issue in validation_result.issues:
                    print(f"   {issue}")
                # Don't fail the test, just warn
                test_issues.append("⚠️ PDF structure validation failed")
        else:
